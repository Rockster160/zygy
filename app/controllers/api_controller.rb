class ApiController < ApplicationController
  ...
  def verify_login_information
    ...
    if params[:identifier_field].present? && params[:password].present?
      ...
      if temp_user
        user = temp_user.valid_password?(params[:password]) ? temp_user.game(game.id) : nil
      end
  ...
  def register_new_user
    server_error = ''
    game = Game.by_identifier(request.headers['Game-Identifier'])
    ...
    if upline
      ...
      user.game(game.id).update(username: params[:username] || "")
      unless user.save
  ...
  def new_purchase
    ...
    if user.present?
      response.headers['Authorization-Success'] = 'true'
      response.headers["Authorization-Code"] = user.generate_authorization_code
      user.purchases.create(amount: (params[:purchase_price].to_f * 100).round)
    else
      response.headers['Authorization-Success'] = 'false'
    end
